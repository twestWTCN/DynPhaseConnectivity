function [plv_dist phi_dist amp_dist segL_ddt consecSegs] = analysestablesegs_PLV_vs_Phi_Amp(PLV,PLV_stable,RP,Amp,period,fsamp)
% if isempty(minsep)
%     minsep = 0;
% end
...
for j = 1:numel(seglist)
    win = tukeywin(size(consecSegs{seglist(j)},2),0.25);
     plv_dist = median(PLV(consecSegs{seglist(j)}));
    for sig = 1:4
