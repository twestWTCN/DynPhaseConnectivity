        ...
        for side = 1:2
            for cond = 1:length(R.condname)
                ...
                if vc_clean.specanaly.flag ~= 1 % Check significant coherences
                    ...
                    ampname = {'HB CTX','HB STN','LB CTX','LB STN'};
                    for i = 1:4
...
%                     end
    ...
    figure(1)
    subplot(1,2,1)
    histogram(x(:),linspace(0.2,0.8,12),'FaceColor',condcr{1},'Normalization','Probability'); %xlim([0 1])
    xlabel('Dwell/Escape'); ylabel('P(X)'); grid on; ylim([0 0.7])
    subplot(1,2,2);
    histogram(y(:),linspace(0.2,0.8,12),'FaceColor',condcr{2},'Normalization','Probability'); %xlim([0 1]);
    xlabel('Dwell/Escape'); ylabel('P(X)'); grid on; ylim([0 0.7])
    clear p
    [h p] = ttest2(x,y);
    title(num2str(p))
